...
public:
    MinHeap(int n, const K& not_found); // Create a min-heap for a max of n pairs (K,V) with notFound returned when empty
    ...
    void decrease_key(const K& key, const V& value); // Decrease value of key
    K remove_min(); // remove and return key with smaller value
};
